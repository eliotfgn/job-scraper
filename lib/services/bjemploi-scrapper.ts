...
export async function scrapeBjEmploi(url: string) {
  ...
  const jobs: JobListing[] = await page.evaluate(() => {
    ...
    return Array.from(jobElements).map((element) => {
      ...
      const locationElement = Array.from(element.querySelectorAll("a.ss_miz_f")).find(el => el.getAttribute("title")?.includes("Lieu d'affectation"));
      const location = locationElement?.textContent?.trim() || "";

      const jobTypeElement = Array.from(element.querySelectorAll("a.ss_miz_f")).find(el => el.getAttribute("title")?.includes("Catégorie - Type de contrat"));
      const jobType = jobTypeElement?.textContent?.trim() || "";

      const startDateElement = element.querySelector("i.fa-calendar");
      const startDate = startDateElement?.nextSibling?.textContent?.trim() || "";
      ...
      const requiredDegreeElement = Array.from(element.querySelectorAll("a.ss_miz_f")).find(el => el.getAttribute("title")?.includes("Diplôme (minimum)"));
